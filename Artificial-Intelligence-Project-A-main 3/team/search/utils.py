...
def line_removal(board):
    ...
    for i in range(BOARD_N):
        ...
        for j in range(BOARD_N):
            if board.get(Coord(i,j)) == PlayerColor.RED or board.get(Coord(i,j)) == PlayerColor.BLUE:
                count += 1
        if count == BOARD_N:
            for j in range(BOARD_N):
                board.pop(Coord(i,j), None) 

    for i in range(BOARD_N):
        count = 0
        for j in range(BOARD_N):
            if board.get(Coord(j,i)) == PlayerColor.RED or board.get(Coord(j,i)) == PlayerColor.BLUE:
                count += 1
        if count == BOARD_N:
            for j in range(BOARD_N):
                board.pop(Coord(j,i), None)
    return board
